...
Copyright © 2019 [Conflux Digital Ltd](https://confluxdigital.net/)
...
We have implemented circuit breakers between our services in order to both isolate them when errors in adjacent components occur. In addition, they can be opened intentionally when needed for testing.

### Evidence

We have a grafana dashboard showing circuit breaker behaviour over the past month, with events detailing how often they have opened, been intentionally opened, automatically recovered or required intervention to close.
...
## OBSERVABILITY: Can the team test both the scheduled and on demand parts of their system
...
* Excellent: _We can test both scheduled and on demand tasks when needed, while they are being developed_
...
* Poor: _Test data and configuration are not in source control, requires a separate team to schedule a change_
* Excellent: _Test data and configuration is in source control and can be set by the team_
...
* Poor: _We are reluctant to change some or part of the codebase without lengthy analysis_
* Excellent: _We are confident that we can make isolatable changes which can realise business value while minimising risk_
...
> We need a store of operational and application knowledge that is kept up to date in order for our stakeholders to integrate with the team which is up to date with our system.
...
* Excellent: _Developers, test, operations and product stakeholders can at least view the code or take part in code reviews_
